...
class ReactGrid extends React.Component {
    ...
    handleTraceSelectorForm = (e) => {
      ...
      let newTraceSelector = this.state.traceSelector
    ...
    handleTraceLabel = (e) =>{
      e.preventDefault()
      let index = parseInt(e.target.id)
      let newTraceLabels = this.state.traceLabels
      let newTraceLabel = e.target.value
    ...
    handleComposedType = (e) => {
      let index = parseInt(e.target.id)
      let newComposedTypes = this.state.composedLines
    ...
    traceOptions(){
     ...
     getCSV(newData){
       let fileName = this.state.downloadFileName
        fetch('/csv', {
            method: "POST",
            credentials: "include",
